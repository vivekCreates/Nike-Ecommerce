...
async function upsertMany(table: any, rows: any[], uniqueKey: string) {
  for (const r of rows) {
    const exists = await db.select().from(table).where(eq(table[uniqueKey], r[uniqueKey])).limit(1);
    if (!exists.length) {
...
async function seed() {
  try {
    ...
    // Fetch inserts for foreign keys
    ...
    const productNames = ['Air Max 1', 'Air Max 2', 'Air Zoom', 'React Runner', 'Pegasus'];
      ...
      for (const color of chosenColors) {
        for (const size of chosenSizes) {
          ...
          const variant = {
            ...
            price: Math.floor(Math.random() * 100) + 80, // integer
            salePrice: null,
            colorId: color.id,
            sizeId: size.id,
